  ...
  methods: {
          ...
          that.courtList = courtConfig.map(function (court) {
            return {
              ...
              times: availableTimes.map(function (t) {
                ...
                return { time: t.time, short: t.short, status: status }
              })
            }
          })
        })
    ...
    loadMyCards() {
      ...
      wx.cloud.callFunction({
        name: 'userApi',
        data: { action: 'getMyCards', userId: userDocId },
        success: function (res) {
          var result = res.result || {}
          that.myCards = result.list || []
        },
        fail: function () {
          that.myCards = []
        },
